...
async def date_dialog(update, context):
    text = update.message.text
    my_message = await send_text(update, context, "Пользователь набирает сообщение...")
...
async def date_button(update, context):
    ...
    await send_html(update, context, "Отличный выбор! Пригласите девушку(парня) за 5 сообщений ")
...
async def message_button(update, context):
    ...
    prompt = load_prompt(query)
    user_chat_history = "\n\n".join(dialog.list)
    my_message = await send_text(update, context, "ChatGPT продумывает варианты ответа...")
...
async def profile_dialog(update, context):
    ...
    if dialog.count == 1:
     ...
     await send_text(update, context, "Ваша цель знакомства?")
    elif dialog.count == 5:
...
async def hello(update, context):
    ...
    if dialog.mode == "opener":
        ...
        await send_text(update, context,'*Добрый день*')
        await send_text(update, context, '_Чем могу помочь?_')
...
app = ApplicationBuilder().token("7511702499:AAEVazX-_hh0MjdMqdboOMBca0cyQLDeu0s").build()
